...
func _ready():
	var file_path = "res://PSD//大鸟.psd"  # 替换为你的 PSD 文件路径
	...
	if psd_file:
		...
		if 压缩方法 == 0:
			...
			while index < 宽度*高度:
				...
				index += 1
			#print(_data)
			var _i = Image.create_from_data(宽度,高度,false,Image.FORMAT_RGBA8,_data)
			var _t = ImageTexture.create_from_image(_i)
			ResourceSaver.save(_t,"res://PSD//保存数据.tres")
		elif 压缩方法 == 1:
			...
			var _i
			if 通道数 == 3:
				_i = Image.create_from_data(宽度,高度,false,Image.FORMAT_RGB8,_data)
			elif 通道数 == 4:
				_i = Image.create_from_data(宽度,高度,false,Image.FORMAT_RGBA8,_data)
			var _t = ImageTexture.create_from_image(_i)
			ResourceSaver.save(_t,"res://PSD//保存数据.tres")
